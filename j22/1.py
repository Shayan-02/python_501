    ...
    try:
        with open(file, 'r', encoding="utf-8") as f:
            ...
            if len(name_list) < num:
                ...
                root2 = Tk()
                root2.title("افراد برنده")
                root2.geometry("300x400")
                winner_label = ttk.Label(root2, text=":افراد برنده", font=font)
                winner_label.pack(pady=15)
                winner_list = [f"{i + 1} : {j}" for i, j in enumerate(winner_list)]
                winners = "\n".join(winner_list)
                show_winner_text = ttk.Label(root2, text=winners, font=font)
                show_winner_text.pack()
                exit_button = ttk.Button(root2, text="بستن", command=root2.destroy)
...
file_label = ttk.Label(
    ...
    font=font, background="#c1c1e1", foreground="black"
).pack(pady=20)


style = ttk.Style()
style.configure("TFrame", font=font, background="#1e90ff", foreground="white")
